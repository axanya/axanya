...
@if (Route::current()->uri() == 'users/security' || Route::current()->uri() == 'users/payout_preferences/{id}' || Route::current()->uri() == 'users/transaction_history')
  ...
  <li>
    <a href="{{ url('users/payout_preferences/'.Auth::user()->user()->id) }}" aria-selected="{{ (Route::current()->uri() == 'users/payout_preferences/{id}') ? 'true' : 'false' }}" class="sidenav-item">{{ trans('messages.header.payout_preferences') }}</a>
  </li>
