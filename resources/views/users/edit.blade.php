...
<div class="panel row-space-4">
  ...
  <div class="panel-body">
      ...
      {!! Form::text('first_name', Auth::user()->user()->first_name, ['id' => 'user_first_name', 'size' => '30', 'class' => 'focus', 'required' => 'required']) !!}
      ...
      {!! Form::text('last_name', Auth::user()->user()->last_name, ['id' => 'user_last_name', 'size' => '30', 'class' => 'focus', 'required' => 'required']) !!}
      ...
      <div class="row row-condensed space-4">
        <label class="text-right col-sm-3" for="user_language">
          {{ trans('messages.footer.choose_language') }}
        </label>
        <div class="col-sm-9">

      <div class="select">
        {!! Form::select('language',$language, (Session::get('language')) ? Session::get('language') : $default_language[0]->value, ['class' => 'language', 'id' => 'language']) !!}
      </div>
        </div>
      </div>

      <div class="row row-condensed space-4">
        <label class="text-right col-sm-3" for="user_currency">
          {{ trans('messages.footer.choose_currency') }}
        </label>
        <div class="col-sm-9">

      <div class="select">
        {!! Form::select('currency',$currency, (Session::get('currency')) ? Session::get('currency') : $default_currency[0]->code, ['class' => 'currency', 'id' => 'currency']) !!}
      </div>
        </div>
      </div>
      ...
      {!! Form::selectRangeWithDefault('birthday_year', date('Y'), date('Y')-120, $dob[0], trans('messages.header.year'), ['id' => 'user_birthday_year', 'class' => 'focus']) !!}
        ...
        <div class="col-sm-9">
          ...
          {!! Form::text('email', Auth::user()->user()->email, ['id' => 'user_email', 'size' => '30', 'class' => 'focus', 'required' => 'required']) !!}
...
<div class="panel row-space-4">
  ...
  <div class="panel-body">
    ...
    <div class="row row-condensed space-4">
      <label class="text-right col-sm-3" for="user_live">{{ trans('messages.profile.where_you_live') }}</label>
      <div class="col-sm-9">
    ...
    <div class="row row-condensed space-4">
      <label class="text-right col-sm-3" for="user_about">{{ trans('messages.profile.describe_yourself') }}</label>
      <div class="col-sm-9">
      ...
      {!! Form::text('work', Auth::user()->user()->work, ['id' => 'user_profile_info_employer', 'size' => '30', 'placeholder' => '', 'class' => 'focus']) !!}
