           ...
           @if(@$expired == 1)
            <li class="media reviews-list-item row-space-top-2">
              <a class="pull-left media-photo media-round" href="{{ url() }}/users/show/{{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->id }}">
                <span class="img-responsive" title="{{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->first_name }}" style="background-image:url({{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->profile_picture->src }});height:60px;width:60px;
                background-size: cover;" alt="{{ $expired_reviews[$i]->review_user(Auth::user()->user()->id)->first_name }}"></span>
              </a>              
              <div class="media-body response">
