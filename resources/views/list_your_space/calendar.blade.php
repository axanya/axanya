  ...
  <h5>{{ trans('messages.lys.saving') }}...</h5>
    ...
    <ul class="list-unstyled row text-center row-space-top-1 display-flex" ng-init="selected_calendar = '{{ lcfirst($result->calendar_type) }}'">
      ...
      <li class="availability-option col-4 display-flex">
        <div data-slug="always" class="option-container available-always {{ ($result->calendar_type == 'Always') ? 'selected_calendar_type' : '' }}" id="available-always">
      ...
      <li class="availability-option col-4 display-flex">
        <div data-slug="sometimes" class="option-container available-sometimes {{ ($result->calendar_type == 'Sometimes') ? 'selected_calendar_type' : '' }}" id="available-sometimes">
      ...
      <li class="availability-option col-4 display-flex">
        <div data-slug="onetime" class="option-container available-onetime {{ ($result->calendar_type == 'Onetime') ? 'selected_calendar_type' : '' }}" id="available-onetime">
    ...
    <div class="modal-cell">
      <div class="modal-content content-container modal_onlycal">
        <div class="panel">
          ...
          <div id="calendar-edit-container" ng-class="selected_calendar == 'always' ? 'hide' : ''"">
...
<button data-href="complete" class="animated btn btn-large btn-host btn-primary {{ ($result->steps_count == 0) ? '' : 'hide'}}" id="js-list-space-button" data-track="list_space_button_left_nav" style="">
 ...
 <div class="col-2 text-right next_step">
    <a data-prevent-default="" href="{{ url('manage-listing/'.$room_id.'/booking') }}" class="btn btn-large btn-primary next-section-button {{ ($result->steps_count != 0) ? '' : 'hide'}}">
