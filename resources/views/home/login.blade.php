        ...
        <div class="panel">
          <div class="panel-body">
            ...
            <a href="{{ $fb_url }}" class="fb-button fb-blue btn icon-btn btn-block btn-large row-space-1 btn-facebook" data-populate_uri="" data-redirect_uri="{{URL::to('/')}}/authenticate">
              <span class="icon-container"><i class="icon icon-facebook"></i></span>
              <span class="text-container">{{ trans('messages.login.login_with') }} Facebook</span>
            </a>

            <a href="{{URL::to('googleLogin')}}" class="btn icon-btn btn-block btn-large row-space-1 btn-google">
              <span class="icon-container"><i class="icon icon-google-plus"></i></span>
              <span class="text-container">{{ trans('messages.login.login_with') }} Google</span>
            </a>
            ...
            {!! Form::open(['action' => 'UserController@authenticate', 'class' => 'signup-form login-form', 'data-action' => 'Signin', 'accept-charset' => 'UTF-8' , 'novalidate' => 'true']) !!}
              ...
              <div class="control-group row-space-1">
                {!! Form::email('email', '', ['class' => $errors->has('email') ? 'decorative-input inspectletIgnore invalid' : 'decorative-input inspectletIgnore', 'placeholder' => trans('messages.login.email_address'), 'id' => 'signin_email', 'style' => 'direction: ltr;']) !!}
                @if ($errors->has('email')) <p class="help-block input-error">{{ $errors->first('email') }}</p> @endif
              </div>

              <div class="control-group row-space-2">
                {!! Form::password('password', ['class' => $errors->has('password') ? 'decorative-input inspectletIgnore invalid' : 'decorative-input inspectletIgnore', 'placeholder' => trans('messages.login.password'), 'id' => 'signin_password', 'data-hook' => 'signin_password', 'style' => 'direction: ltr;']) !!}
