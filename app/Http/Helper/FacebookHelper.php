...
class FacebookHelper {
	...
	public function getUrlLogin() {
		return $this->helper->getLoginUrl(url('facebookAuthenticate'), ['public_profile', 'email', 'user_friends', 'user_birthday']);
	}
	...
	public function getUrlConnect() {
		return $this->helper->getLoginUrl(url('facebookConnect'), ['public_profile', 'email', 'user_friends', 'user_birthday']);
	}
	...
	public function getData() {
		...
		try {
			...
			$response = $this->fb->get('/me?fields=id,first_name,last_name,friends,email,birthday');
	...
	public function getMutualFriends() {
		// Sets the default fallback access token so we don't have to pass it to each request
		$token = $this->fb->setDefaultAccessToken($_SESSION['facebook_access_token']);
		try {
			/*$response = $this->fb->get('/me?fields=id,first_name,last_name,friends,email,birthday');
				echo "<pre>";
			*/
			// $response = $this->fb->get('/1231320426892210?fields=id,first_name,last_name,friends,email,context.fields(all_mutual_friends)');
			$response = $this->fb->get('/1307031059372864?fields=context.fields%28mutual_friends%29');
			//$userNode = $response->getGraphUser();
			echo "<pre>";
