...
$(document).on('change', '[id^="calendar-select-"], [id^="select-"]', function() {
	var data_params = {};
	...
	$http.post('update_rooms_policies', { data:data }).then(function(response) {
		...
		if(response.data.success == 'true') {
			$('.'+saving_class+' h5').text('Saved!');
	 		...
	 		$('#steps_count').text(response.data.steps_count);
	 		$scope.steps_count = response.data.steps_count;
			$scope.house_rules_status = $scope.suitable_for_children != '0' && $scope.suitable_for_infants != '0' && $scope.suitable_for_pets != '0' && $scope.smoking_allowed != '0' && $scope.parties_allowed != '0'
	 		if ( $scope.cancel_policy != '' && $scope.house_rules_status ) {
		    ...
		    $('[data-track="terms"] a div div .pull-right .nav-icon').removeClass('hide');
				...
				$('[data-track="terms"] a div div .transition').removeClass('hide');
				$('[data-track="terms"] a div div .pull-right .nav-icon').addClass('hide');
			}
		}
	});
...
$(document).on('change', '[name="is_referral"], [name="referral_code"]', function()
{
	...
	$http.post('update_rooms', { data : data}).then(function(response)
	{
		if(response.data.success == 'true')
		{
			console.log($scope.phone_verification_status);
			$('.'+saving_class+' h5').text('Saved!');
     		$('.'+saving_class).fadeOut();

     		$('#steps_count').text(response.data.steps_count);
     		$scope.steps_count = response.data.steps_count;
				// && $scope.phone_verification_status == 'Confirmed'
     		if ( ( referral_code.length > 0 || status == 'No' ) && $('#listing-verification').length < 1 ) {
		    	...
		    	$('[data-track="verification"] a div div div .icon-ok-alt').removeClass('hide');
     		} else {
     			$('[data-track="verification"] a div div .transition').removeClass('hide');
    			$('[data-track="verification"] a div div div .icon-ok-alt').addClass('hide');
     		}
     	}
    });
...
$(document).on('click', '[id^="mobile-verification-"]', function()
{
	...
	var $progress = $('.saving-progress').filter(':first');

	$progress.find('h5').text(first_msg);
	...
	$http.post('mobile_verification', post_data).then(function(response)
	{
		if(response.data.success == 'true')
		{
			...
			if( data_value == 'verify' ) {
				...
				if ( ( $scope.referral_code != '' || $scope.is_referral ) && $('#listing-verification').length < 1 ) {
					$('[data-track="verification"] a div div .transition').removeClass('visible');
			    $('[data-track="verification"] a div div .transition').addClass('hide');
			...
			$progress.find('h5').text(last_msg);
     		$progress.fadeOut();
     		$('#steps_count').text(response.data.steps_count);
     		$scope.steps_count = response.data.steps_count;
...
$(document).on('click', '[id^="change_number"]', function()
{
    ...
    $('#change_phone_number').show();
		$('#change_phone_number #phone_number').trigger('focus');
});
...
$(document).on('submit', 'form#veryfyotp4', function(e) {
	$(this).find('button#mobile-verification-change-number').trigger('click');
	e.preventDefault();
	return false;
});

$(document).on('submit', 'form#veryfyotp', function(e) {
	$(this).find('button#mobile-verification-verify-code').trigger('click');
	e.preventDefault();
	return false;
});
...
$(document).on('submit', '#dashboard-change-number', function(e) {
	e.preventDefault();
	var $form = $(this);
...
$(document).on('submit', '#dashboard-verify-number', function(e) {
	e.preventDefault();
	var $form = $(this);
	$.ajax({
		url: '/mobile_verification',
...
$(document).on('submit', '#form_update_email', function(e) {
	var $form = $(this);
	$.ajax({
		...
		dataType: 'json',
		beforeSend: function() {
			$form.closest('.dashboard-popup').find('#update_email_status').text('Updating email...').show();
		},
		success: function(response) {
			console.log(response);
			if(response.success) {
				if(response.changed) {
					$form.closest('.dashboard-popup').find('[data-action="close"]').trigger('click');
					$(document).find('[data-popup="email-otp"]').trigger('click');
				} else {
					$form.closest('.dashboard-popup').find('#update_email_status').text('Email updated successfully.').show();
				}
			} else {
				alert(response.error);
				$form.closest('.dashboard-popup').find('#update_email_status').text(response.error).show();
			}
		}
	});
	e.preventDefault();
...
$(document).on('submit', '#form-verify-email-otp', function(e) {
	var $form = $(this);
	$.ajax({
		url: '/users/confirm_email_by_otp',
		method: 'get',
		data: {
			code: $form.find('#input-email-otp').val()
		},
		dataType: 'json',
		beforeSend: function() {
			$('#email-otp-verification-status').text('Verifying...').show();
		},
		success: function(response) {
			console.log(response);
			if(response.success) {
				$('#email-otp-verification-status').text('Verification successful.').show();
				window.location.reload(true);
			} else {
				$('#email-otp-verification-status').hide();
				alert(response.error);
			}
		}
	});
...
$(window).on('popupdisplayed', function(e, data) {
	...
	if(id === 'phone-verification') {
		...
		if(hidden_phone_number != '' && hidden_phone_status == 'Pending') {
			$('#dashboard-verify-number').show();
			$('#dashboard-change-number').hide();
		}

	} else if (id === 'email-otp') {
		$('#form-verify-email-otp').hide();
		$('#otp-sending').show();
		$.ajax({
			url: '/users/request_email_otp',
			dataType: 'json',
			success: function(response) {
				if(response.success) {
					$('#otp-sending').hide();
					$('#form-verify-email-otp').show();
					$('#form-verify-email-otp').trigger('reset');
					$('#form-verify-email-otp').find('#input-email-otp').trigger('focus');
				}
			}
		});
	}
