...
$scope.reply_message = function(value) {
  ...
  $http.post(APP_URL+'/messaging/qt_reply/'+$('#reservation_id').val(), { message: message, template: template, pricing_room_id: $('#pricing_room_id').val(), pricing_checkin: $('#pricing_start_date').val(), pricing_checkout: $('#pricing_end_date').val(), pricing_guests: $('#pricing_guests').val(), pricing_price: $('#pricing_price').val() }).then(function(response) {
...
function calculation(checkout,checkin,guest,room_id) {
    ...
    $http.post('../../rooms/price_calculation', { checkin :checkin,checkout : checkout, guest_count : guest,   room_id : room_id }).then(function(response) {
