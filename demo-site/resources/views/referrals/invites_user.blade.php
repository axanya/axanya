...
@section('main')
    <main id="site-content" role="main" ng-controller="footer">
        ...
        <section class="page-container-responsive row-space-top-3 row-space-6">
            ...
            <input type="text" class="typeahead input-large tt-hint" autocorrect="off" readonly="" autocomplete="off"
            ...
            <input id="email-list" type="text" class="typeahead input-large tt-input" autocorrect="off"
                   placeholder="{{ trans('messages.referrals.add_friends_email') }}" autocomplete="off"
                   spellcheck="false" dir="auto"
                   style="position: relative; vertical-align: top; background-color: white;"
            ><pre aria-hidden="true"
                  style="position: absolute; visibility: hidden; white-space: pre; font-family: Circular, 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 16px; font-style: normal; font-variant: normal; font-weight: 400; word-spacing: 0px; letter-spacing: 0px; text-indent: 0px; text-rendering: auto; text-transform: none;"></pre>
            <div class="tt-menu" style="position: absolute; top: 100%; left: 0px; z-index: 100; display: none;"><div
                        class="tt-dataset tt-dataset-email-autocomplete"></div></div>
            </span>
                                </div>
                                <a href="javascript:void();" class="btn btn-primary btn-large input-suffix"
                                   id="send-email">{{ trans('messages.inbox.send') }}</a>
                            </div>
            ...
            <span id="info_message">
              ...
              {{ trans('messages.referrals.invitation_sent') }}!
                        ...
                        <div class="col-sm-12 col-md-6 col-lg-5 col-lg-push-1 referral-link-share">
                            <div class="input-addon">
                                <input id="share-link" class="input-large" type="text" value="{{ url('c/'.$username) }}"
                                       readonly="">

                                <a class="btn btn-primary btn-large input-suffix share-btn btn-facebook"
                                   data-network="facebook" rel="nofollow" title="Facebook"
                                   href="http://www.facebook.com/sharer.php?u={{ url('c/'.$username) }}"
                                   target="_blank">
  ...
  <span class="share-title">
    {{ trans('messages.rooms.share') }}:
      ...
      <a class="share-btn link-icon" data-network="twitter" rel="nofollow" title="Twitter"
         href="https://twitter.com/intent/tweet?source=tweetbutton&amp;text=Travel+on+{{ $site_name }}+and+get+{{ $result->value(5) }}{{ $result->value(4) }}+in+travel+credit!+{{ '@'.$site_name }}&amp;url={{ url('c/'.$username) }}"
         target="_blank">
                        ...
                        <div>
                            @foreach($referrals as $row)
                                <div class="media sent-referral">
                                    <div class="hide-sm">
                                        <img class="media-photo media-round pull-left referred-profile-pic"
                                             src="{{ $row->friend_users->profile_picture->header_src }}"
                                             alt="{{ $row->friend_users->first_name }}" width="50" height="50">
                                    </div>
