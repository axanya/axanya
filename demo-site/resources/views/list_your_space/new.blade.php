                            ...
                            <div class="col-lg-9 col-md-11 col-md-push-1 col-lg-push-0" style="left:20%;">
                                ...
                                <div class="row fieldset fieldset_property_type_id">
                                    <div class="btn-group" ng-hide="property_type_id">
                                        <button class="btn btn-large type alert-highlighted-element hover-select-highlight"
                                                data-name="{{ $property_type[0]->name }}"
                                                data-type-id="{{ $property_type[0]->id }}"
                                                data-icon-class="icon-apartment" data-behavior="tooltip" type="button"
                                                data-position="bottom"
                                                aria-label="Your space is an apartment, flat, or unit in a multi-unit building."
                                                ng-click="property_type('{{ $property_type[0]->id }}', '{{ $property_type[0]->name }}', 'icon-apartment')">
                                            <i class="icon icon-apartment h4 icon-kazan"></i>
                                            {{ $property_type[0]->name }}
                                        </button>
                                        <button class="btn btn-large type alert-highlighted-element hover-select-highlight"
                                                data-name="{{ $property_type[1]->name }}"
                                                data-type-id="{{ $property_type[1]->id }}" data-icon-class="icon-home"
                                                data-behavior="tooltip" type="button" data-position="bottom"
                                                aria-label="Your space is a single-family house or townhouse."
                                                ng-click="property_type('{{ $property_type[1]->id }}', '{{ $property_type[1]->name }}', 'icon-home')">
                                            <i class="icon icon-home h4 icon-kazan"></i>
                                            {{ $property_type[1]->name }}
                                        </button>
                                        <button class="btn btn-large type alert-highlighted-element hover-select-highlight"
                                                data-name="{{ $property_type[2]->name }}"
                                                data-type-id="{{ $property_type[2]->id }}" data-icon-class="icon-cup"
                                                data-behavior="tooltip" type="button" data-position="bottom"
                                                aria-label="You rent out several rooms within an establishment. Your service includes breakfast."
                                                ng-click="property_type('{{ $property_type[2]->id }}', '{{ $property_type[2]->name }}', 'icon-cup')">
                                            <i class="icon icon-cup h4 icon-kazan"></i>
                                            {{ $property_type[2]->name }}
                                        </button>
                                        <div class="select select-large other-select" id="property-select">
                                            <select class="alert-highlighted-element hover-select-highlight"
                                                    id="property_type_dropdown" ng-model="change_property_value"
                                                    ng-change="property_change(change_property_value)">
                                                <option disabled="" selected=""
                                                        value="">{{ trans('messages.profile.other') }}</option>
                                                {{--*/ $i = 0 /*--}}
                                                @foreach($property_type as $row)
                                                    @if($i > 2)
                                                        <option data-icon-class="icon-star-alt"
                                                                data-name="{{ $row->name }}"
                                                                data-type-id="{{ $row->id }}" value="{{ $row->id }}">
                                                            {{ $row->name }}
                                                        </option>
                                                    @endif
                                                    {{ $i++ }}
                                    ...
                                    <div class="active-selection" ng-show="property_type_id"
                                         ng-click="property_type_rm()" style="display:none;">
                                ...
                                <div class="row fieldset fieldset_room_type">
                                    <div class="btn-group" ng-hide="room_type_id">
                                        ...
                                        <button class="btn btn-large type alert-highlighted-element hover-select-highlight"
                                                data-name="{{ $room_type[0]->name }}" data-slug="entire_home_apt"
                                                data-type="{{ $room_type[0]->name }}"
                                                data-type-id="{{ $room_type[0]->id }}"
                                                data-icon-class="icon-entire-place" data-behavior="tooltip"
                                                type="button" data-position="bottom"
                                                aria-label="You&#39;re renting out an entire home."
                                                ng-click="room_type('{{ $room_type[0]->id }}', '{{ $room_type[0]->name }}', 'icon-entire-place')">
                                        ...
                                        <button class="btn btn-large type alert-highlighted-element hover-select-highlight"
                                                data-name="{{ $room_type[1]->name }}" data-slug="private_room"
                                                data-type="{{ $room_type[1]->name }}"
                                                data-type-id="{{ $room_type[1]->id }}"
                                                data-icon-class="icon-private-room" data-behavior="tooltip"
                                                type="button" data-position="bottom"
                                                aria-label="You&#39;re renting out a private room within a home."
                                                ng-click="room_type('{{ $room_type[1]->id }}', '{{ $room_type[1]->name }}', 'icon-private-room')">
                                        ...
                                        <button class="btn btn-large type alert-highlighted-element hover-select-highlight"
                                                data-name="{{ $room_type[2]->name }}" data-slug="shared_room"
                                                data-type="{{ $room_type[2]->name }}"
                                                data-type-id="{{ $room_type[2]->id }}"
                                                data-icon-class="icon-shared-room" data-behavior="tooltip" type="button"
                                                data-position="bottom"
                                                aria-label="You&#39;re renting out a common area, such as an airbed in a living room."
                                                ng-click="room_type('{{ $room_type[2]->id }}', '{{ $room_type[2]->name }}', 'icon-shared-room')">
                                    ...
                                    <div class="active-selection" ng-show="room_type_id" ng-click="room_type_rm()"
                                         style="display:none;">
                                ...
                                <div class="row fieldset fieldset_person_capacity wmpw-spacing">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-sm-5">
                                                            <div class="select select-large">
                                                                <select id="accomodates-select"
                                                                        class="hover-select-highlight"
                                                                        ng-model="accommodates_value"
                                                                        ng-change="change_accommodates(accommodates_value)">
                                                                    @for($i=1;$i<=16;$i++)
                                    ...
                                    <div class="active-selection" ng-show="selected_accommodates"
                                         ng-click="accommodates_rm()" style="display:none;">
                                ...
                                <div class="row fieldset fieldset_city">
                                            ...
                                            <div class="panel location lys-location alert-highlighted-element">
                                                <div class="panel-body panel-light panel-border location-panel-body clearfix">
                                                    <div class="col location-input-frame">
                                                        ...
                                                        <input class="pull-left alert-highlighted-element geocomplete"
                                                               name="location_input" type="text" value=""
                                                               placeholder="Enter a location" autocomplete="off"
                                                               ng-click="city_click()" ng-model="address"
                                                               id="location_input">
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <div class="active-selection" ng-show="city_show" ng-click="city_rm()"
                                         style="display:none;">
                                ...
                                <div class="row text-left space-top-3">
                                    <div class="" id="js-submit-button">
                                        <div class="lys-continue-button-wrapper col-3">
                                            <button type="submit" class="btn btn-primary btn-large btn-block submit"
                                                    ng-disabled="((city_show == false) || lys_new.$invalid)">
                    ...
                    <div class="page-container-responsive col-center">
                        <div class="row space-5">
                            ...
                            <div class="col-md-4 text-center">
                                ...
                                <div class="text-lead text-color-light">
                                    {{ trans('messages.lys.host_guarantee_desc1') }} <a
                                            href="{{ url('host_guarantee') }}"
                                            target="_blank">{{ trans('messages.lys.eligible') }}</a> {{ trans('messages.lys.host_guarantee_desc2',['site_name'=>$site_name]) }}
                                </div>
                            </div>
                            <div class="col-md-4 text-center">
