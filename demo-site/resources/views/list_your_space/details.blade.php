<div class="manage-listing-content-container" id="js-manage-listing-content-container">
    <div class="manage-listing-content-wrapper">
        <div class="manage-listing-content no-top-padding" id="js-manage-listing-content">
            <div>
                ...
                <div class="js-section" id="js-section-details"
                     ng-init="space='{{ $result->rooms_description->space }}'; access='{{ $result->rooms_description->access }}'; interaction='{{ $result->rooms_description->interaction }}'; notes='{{ $result->rooms_description->notes }}'; house_rules='{{ $result->rooms_description->house_rules }}'; neighborhood_overview='{{ $result->rooms_description->neighborhood_overview }}'; transit='{{ $result->rooms_description->transit }}';">
                    ...
                    <div class="row-space-2">
                        <label for="details_later" id="help-panel-details_later">
                            <input type="checkbox" name="details_later" value="1"
                                   id="details_later" {{ ($result->rooms_description->details_later == 1) ? 'checked' : '' }} >
                    ...
                    <div class="row-space-2" id="help-panel-space">
                        ...
                        <textarea class="input-large textarea-resize-vertical" name="space" rows="4"
                                  placeholder="{{ trans('messages.lys.space_placeholder') }}"
                                  ng-model="space">{{ $result->rooms_description->space }}</textarea>
                    </div>
                    <div class="row-space-2" id="help-panel-access">
                        <label class="label-large">{{ trans('messages.lys.guest_access') }}</label>
                        <textarea class="input-large textarea-resize-vertical" name="access" rows="4"
                                  placeholder="{{ trans('messages.lys.guest_access_placeholder') }}"
                                  ng-model="access">{{ $result->rooms_description->access }}</textarea>
                    </div>
                    <div class="row-space-2" id="help-panel-interaction">
                        <label class="label-large">{{ trans('messages.lys.interaction_with_guests') }}</label>
                        <textarea class="input-large textarea-resize-vertical" name="interaction" rows="4"
                                  placeholder="{{ trans('messages.lys.interaction_with_guests_placeholder') }}"
                                  ng-model="interaction">{{ $result->rooms_description->interaction }}</textarea>
                    </div>
                    <div class="row-space-2" id="help-panel-notes">
                        <label class="label-large">{{ trans('messages.lys.other_things_note') }}</label>
                        <textarea class="input-large textarea-resize-vertical" name="notes" rows="4"
                                  placeholder="{{ trans('messages.lys.other_things_note_placeholder') }}"
                                  ng-model="notes">{{ $result->rooms_description->notes }}</textarea>
                    </div>
                    <div class="row-space-2" id="help-panel-house-rules">
                        <label class="label-large">{{ trans('messages.lys.house_rules') }}</label>
                        <textarea class="input-large textarea-resize-vertical" name="house_rules" rows="4"
                                  placeholder="{{ trans('messages.lys.house_rules_placeholder') }}"
                                  ng-model="house_rules">{{ $result->rooms_description->house_rules }}</textarea>
                    </div>
                ...
                <div class="js-section">
                    ...
                    <div class="row-space-2" id="help-panel-neighborhood">
                        ...
                        <textarea class="input-large textarea-resize-vertical" name="neighborhood_overview" rows="4"
                                  placeholder="{{ trans('messages.lys.overview_placeholder') }}"
                                  ng-model="neighborhood_overview">{{ $result->rooms_description->neighborhood_overview }}</textarea>
                    </div>
                    <div id="help-panel-transit">
                        <label class="label-large">{{ trans('messages.lys.getting_around') }}</label>
                        <textarea class="input-large textarea-resize-vertical" name="transit" rows="4"
                                  placeholder="{{ trans('messages.lys.getting_around_placeholder') }}"
                                  ng-model="transit">{{ $result->rooms_description->transit }}</textarea>
                    </div>
                ...
                <div class="not-post-listed row row-space-top-6 progress-buttons">
                    ...
                    @if($result->status == NULL)
                        ...
                        <div class="col-2 text-right next_step">
                            <a data-prevent-default="" href="{{ url('manage-listing/'.$room_id.'/location') }}"
