...
@section('main')
  ...
  <a href="{{ url('invite?r=5') }}" class="btn btn-block">Invite Friends</a>
                            ...
                            <div class="panel-body" id="payout_intro">
                                ...
                                <table class="table table-striped" id="payout_methods">
                                                ...
                                                <td class="payout-options">
                                                    @if($row->default != 'yes')
                                                        <li class="dropdown-trigger list-unstyled">
                                                            <a data-prevent-default="" href="javascript:void(0);"
                                                               class="link-reset text-truncate"
                                                               id="payout-options-{{ $row->id }}">
                                                                {{ trans('messages.your_trips.options') }}
                                                                <i class="icon icon-caret-down"></i>
                                                            </a>
                                                            <ul data-sticky="true"
                                                                data-trigger="#payout-options-{{ $row->id }}"
                                                                class="tooltip tooltip-top-left list-unstyled dropdown-menu"
                                                                aria-hidden="true">
                                                                <li>
                                                                    <a rel="nofollow" data-method="post"
                                                                       class="link-reset menu-item"
                                                                       href="{{ url() }}/users/payout_delete/{{ $row->id }}">{{ trans('messages.account.remove') }}</a>
                                                                </li>
                                                                <li>
                                                                    <a rel="nofollow" data-method="post"
                                                                       class="link-reset menu-item"
                                                                       href="{{ url() }}/users/payout_default/{{ $row->id }}">{{ trans('messages.account.set_default') }}</a>
                                                                </li>
                                                            </ul>
                                                        </li>
                                                    @endif
                                    ...
                                    <tr id="add_payout_method_section">
                                        <td colspan="4">
                                            <a id="add-payout-method-button" class="btn btn-primary"
                ...
                <div id="modal-add-payout-set-address" class="modal-content">
                    ...
                    <form class="modal-add-payout-pref" method="post" id="address">
                        ...
                        <div class="panel-body">
                            <div>
                                <label for="payout_info_payout_address1">{{ trans('messages.account.address') }}
                                    *</label>
                                <input type="text" autocomplete="billing address-line1" id="payout_info_payout_address1"
                                       name="address1">
                            </div>
                            <div>
                                <label for="payout_info_payout_address2">{{ trans('messages.account.address') }} 2
                                    / {{ trans('messages.account.zone') }}</label>
                                <input type="text" autocomplete="billing address-line2" id="payout_info_payout_address2"
                                       name="address2">
                            </div>
                            <div>
                                <label for="payout_info_payout_city">{{ trans('messages.account.city') }}*</label>
                                <input type="text" autocomplete="billing address-level2" id="payout_info_payout_city"
                                       name="city">
                            </div>
                            <div>
                                <label for="payout_info_payout_state">{{ trans('messages.account.state') }}
                                    / {{ trans('messages.account.province') }}</label>
                                <input type="text" autocomplete="billing address-level1" id="payout_info_payout_state"
                                       name="state">
                            </div>
                            <div>
                                <label for="payout_info_payout_zip">{{ trans('messages.account.postal_code') }}*</label>
                                <input type="text" autocomplete="billing postal-code" id="payout_info_payout_zip"
                                       name="postal_code">
                            </div>
                            <div>
    ...
    <div class="modal hide" id="payout_popup2" aria-hidden="false" style="" tabindex="-1">
        <div class="modal-table">
            <div class="modal-cell">
                <div id="modal-add-payout-set-address" class="modal-content">
                    <div class="panel-header">
                    ...
                    <form class="modal-add-payout-pref" id="country_options" accept-charset="UTF-8">
                        ...
                        <div class="panel-footer">
                            <input type="submit" value="{{ trans('messages.account.next') }}"
                                   id="select-payout-method-submit" class="btn btn-primary">
                        </div>
                    </form>
        ...
        <div class="modal-table">
            <div class="modal-cell">
                <div id="modal-add-payout-set-address" class="modal-content">
                    ...
                    <form method="post" id="payout_paypal"
                          action="{{ url('users/payout_preferences/'.Auth::user()->user()->id) }}"
                        ...
                        <div class="panel-footer">
                            <input type="submit" value="{{ trans('messages.account.submit') }}" id="modal-paypal-submit"
    ...
    @foreach($payouts as $row)
        <ul data-sticky="true" data-trigger="#payout-options-{{ $row->id }}"
            class="tooltip tooltip-top-left list-unstyled dropdown-menu" aria-hidden="true" role="tooltip"
            style="left: 1019.45px; top: 232.967px;">
            @if($row->default != 'yes')
                <li>
                    <a rel="nofollow" data-method="post" class="link-reset menu-item"
                       href="{{ url() }}/users/payout_delete/{{ $row->id }}">{{ trans('messages.account.remove') }}</a>
                </li>
            @endif
            <li>
