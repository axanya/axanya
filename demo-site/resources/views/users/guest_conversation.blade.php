...
@section('main')
    ...
    <main id="site-content" role="main" ng-controller="conversation">
                        ...
                        <div>
                            ...
                            @if($messages[0]->message_type == 2)
                                <div class="text-center panel-body banner-status space-6">
                                    <div class="h4 space-1">
                                        <strong>
                                            <span>{{ trans('messages.inbox.reservation_confirmed_place') }} {{ $messages[0]->reservation->rooms->rooms_address->city }}
                                                , {{ $messages[0]->reservation->rooms->rooms_address->country_name }}</span>
                                        </strong>
                                    </div>
                                    <a href="{{ url() }}/reservation/itinerary?code={{ $messages[0]->reservation->code }}"
                                       class="btn space-top-3">
        ...
        <span>Change or Cancel</span>
                            ...
                            @if($messages[0]->message_type == 3)
                                <div class="text-center panel-body banner-status space-6">
                                    ...
                                    <a class="btn space-top-3"
                                       href="{{ url() }}/s?location={{ $messages[0]->reservation->rooms->rooms_address->city }}">
                                ...
                                <div class="panel action-status row-space-6">
                                    <div class="panel-body text-center">
                                        ...
                                        <div class="space-top-3">
                                            <a href="{{ url() }}/payments/book?checkin={{ $messages[0]->special_offer->checkin }}&amp;checkout={{ $messages[0]->special_offer->checkout }}&amp;room_id={{ $messages[0]->special_offer->room_id }}&amp;number_of_guests={{ $messages[0]->special_offer->number_of_guests }}&amp;ref=qt2_preapproved&amp;special_offer_id={{ $messages[0]->special_offer_id }}"
                            ...
                            @endif
                            <div id="post_message_box" data-key="guest_conversation"
                                 class="row row-condensed row-space-6 send-message-box">
                                <div class="col-sm-10">
                                    <div class="panel-quote-flush panel-quote panel-quote-right panel">
                                        <div class="panel-body text-left text-medium-gray"><textarea rows="3"
                                                                                                     placeholder=""
                                                                                                     class="send-message-textarea"
                                                                                                     id="message_text"
                                                                                                     name="message"></textarea>
                                        </div>
                                        <div class="panel-body panel-dark text-right">
                                            <button class="btn" id="reply_message"
                                                    ng-click="reply_message('guest_conversation')">{{ trans('messages.your_reservations.send_message') }}</button>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-sm-2 text-right">
                                    <div class="media-photo media-round" style="width:70px; height:70px;"><span
                                                style="background-image:url({{ Auth::user()->user()->profile_picture->src }}); width:70px; height:70px;"></span>
                                    </div>
                                </div>
                            </div>
                            <div id="thread-list">
        ...
        <span>{{ $messages[$i]->special_offer->currency->symbol.$messages[$i]->special_offer->price }}
            /{{ ucfirst(trans_choice('messages.inbox.night',1)) }}</span>
        </span>
        </span>
                                                    </div>
                                                </div>
                                                        ...
                                                        <div class="row row-condensed row-space-6 post">
                                                            <div class="col-sm-2 text-left">
                                                                <div class="media-photo media-round"
                                                                    ...
                                                                    <span style="background-image:url({{ $messages[$i]->user_details->profile_picture->src }}); width:70px; height:70px;"
                                                                        ...
                                                                        <div>
                                                                            <div>
                                                                                <div class="pull-right">
                                                                                    <a data-prevent-default="true"
                                                                                       title="Report this message"
                                                                                       class="flag-trigger link-reset"
                                                                                       href="#">
                                                    ...
                                                    <div>
                                                        <div class="row row-condensed row-space-6 post">
                                                            ...
                                                            <div class="col-sm-2 text-right">
                                                                <div class="media-photo media-round"
                                                                     style="width:70px; height:70px;">
                                                                    <span style="background-image:url({{ Auth::user()->user()->profile_picture->src }}); height:70px; width:70px;"
                                                                          class="user-profile-photo"></span>
                                                                </div>
                                                            </div>
                                                        </div>
                    ...
                    <form accept-charset="UTF-8"
                        ...
                        <div class="text-center mini-profile media">
                            ...
                            <div class="space-top-3 text-center">
                                <div class="h4">
                                    <a href="{{ url() }}/users/show/{{ $messages[0]->reservation->rooms->users->id }}"
                                       class="text-normal">{{ $messages[0]->reservation->rooms->users->first_name }}</a>
                                </div>
                                    ...
                                    <div class="row-space-6 row-space-top-6">
                                        <div class="qt-reservation-info clearfix">
                                            <div class="row-space-6 h4">
                                                <a class="text-normal"
                                                   href="{{ url() }}/rooms/{{ $messages[0]->reservation->room_id }}">{{ $messages[0]->reservation->rooms->name }}</a>
                                            </div>
