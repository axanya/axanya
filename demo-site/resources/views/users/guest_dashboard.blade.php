    ...
    <main id="site-content" role="main">
            ...
            <div class="row">
             ...
             dashboard-profile-photo panel-image ">
                            <a href="{{ url('users/show/'.Auth::user()->user()->id) }}"
                               title="{{ trans('messages.dashboard.view_profile') }}">
                                ...
                                @if(Auth::user()->user()->profile_picture->src == '' || Auth::user()->user()->about == '')
                                    <li>
                                        <a href="{{ url('users/edit') }}"
                                           class="btn btn-primary btn-block text-wrap space-top-1"
                                           id="edit-profile">{{ trans('messages.dashboard.complete_profile') }}</a>
                                    </li>
                                @endif
                            </ul>
                        </div>
      ...
      {{ trans('messages.dashboard.add_verifications') }}
              ...
              <span class="dashboard_alert_text">
                {{ trans('messages.dashboard.confirm_your_email') }} <a
                          href="{{ url('users/request_new_confirm_email') }}">{{ trans('messages.dashboard.request_confirmation_email') }}</a> {{ trans('messages.login.or') }}
                  <a href="{{ url('users/edit') }}">{{ trans('messages.dashboard.change_email_address') }}</a>.
              </span>
                                                </div>
                    ...
                    <div class="panel space-4">
                        <div class="panel-header">
                            ...
                            ({{$all_message->count()}} {{ trans('messages.dashboard.new') }})
                        </div>
                                ...
                                <li id="thread_{{ $all->id }}" class="panel-body thread-read thread">
                                    <div class="row">
                                        <div class="col-3">
                                            <div class="row row-table">
                                                <div class="col-5">
                                                    <a data-popup="true" href="#"><span
                                                                title="{{ $all->user_details->first_name }}"
                                                                style="background-image:url({{ $all->user_details->profile_picture->src }}); height:50px; width:50px;"
            ...
            <span class="street-address">{{ $all->rooms_address->address_line_1 }} {{ $all->rooms_address->address_line_2 }}</span>, <span
                                                                                class="locality">{{ $all->rooms_address->city }}</span>, <span
                                                                                class="region">{{ $all->rooms_address->state }}</span>
          ({{  (date('M d', strtotime( $all->reservation->checkin))) }}
                                                                        , {{  (date('M d, Y', strtotime( $all->reservation->checkout))) }}
                                                                        )
        </span>
                                                                </div>
                                                            </a>
