    ...
    <footer class="page-container-responsive" ng-controller="footer">
        <div class="row row-condensed">
                ...
                <div class="language-curr-picker clearfix">
                    ...
                    <div class="select select-large select-block row-space-2">
                        <label id="currency-selector-label"
                               class="screen-reader-only">{{ trans('messages.footer.choose_currency') }}</label>
            ...
            <div class="col-md-2 col-md-offset-1 hide-sm">
                ...
                <ul class="list-layout">
                    <li><a href="{{ url('contact') }}" class="link-contrast">{{ trans('messages.footer.contact') }}</a>
                    </li>
                    @foreach($company_pages as $company_page)
                        <li><a href="{{ url($company_page->url) }}" class="link-contrast">{{ $company_page->name }}</a>
                        </li>
                    @endforeach
            ...
            <div class="col-md-2 col-md-offset-1 hide-sm">
                ...
                <ul class="list-layout">
                    <li><a href="{{ url('invite') }}"
                           class="link-contrast">{{ trans('messages.footer.invite_friends') }}</a></li>
                    @foreach($discover_pages as $discover_page)
                        <li><a href="{{ url($discover_page->url) }}"
                               class="link-contrast">{{ $discover_page->name }}</a></li>
                    @endforeach
                </ul>
            </div>
            ...
            <div class="col-md-2 col-md-offset-1 hide-sm">
                ...
                <ul class="list-layout">
                    @foreach($hosting_pages as $hosting_page)
                        <li><a href="{{ url($hosting_page->url) }}"
                               {{$hosting_page->url == 'kosher' ?'target=_blank':''}} class="link-contrast">{{ $hosting_page->name }}</a>
                        </li>
                    @endforeach
                </ul>
            </div>
        ...
        <div class="footer-table">
            <ul class="list-layout list-inline icon-list" itemscope="" itemtype="http://schema.org/Organization">
                <i class="footer-logoicon"
                   style="{{ (!isset($exception)) ? (Route::current()->uri() == '/' ? $home_logo_style : $logo_style) : $logo_style }}"></i>
                <div class="text-muted">
                    © {{ $site_name }}, Inc.
                </div>
            </ul>
            <ul class="list-layout list-inline icon-list right-list" itemscope=""
                ...
                <div class="hide-sm">
                    ...
                    @for($i=0; $i<count($join_us); $i++)
                        @if($join_us[$i]->value != '')
                            <li>
                                <a href="{{ $join_us[$i]->value }}" class="link-contrast footer-icon-container"
                                   target="_blank">
