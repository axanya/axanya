...
@section('main')
    <main id="site-content" role="main" ng-controller="wishlists">
                                            ...
                                            <div class="listing">
                                                <div class="panel-image listing-img img-large">
                                                    ...
                                                    <a href="{{ url('rooms/'.$row->id) }}"
                                                       class="media-photo media-cover wishlist-bg-img" target="_blank">
                                                        <img src="{{ url('images/'.$row->photo_name) }}" width="639"
                                                             height="426">
                                                    </a>
      ...
      <span class="rich-toggle wish_list_button wishlist-button not_saved" data-hosting_id="{{ $row->id }}"
            data-img="{{ url('images/'.$row->photo_name) }}" data-name="{{ $row->name }}"
            data-address="{{ $row->rooms_address->city }}" title="Save to Wish List">
        <input type="checkbox" id="wishlist-widget-{{ $row->id }}" name="wishlist-widget-{{ $row->id }}"
               data-for-hosting="{{ $row->id }}">
        <label for="wishlist-widget-{{ $row->id }}">
          ...
          <i class="icon icon-heart-alt icon-white icon-size-2" id="wishlist-widget-icon-{{ $row->id }}"
             data-room_id="{{ $row->id }}" data-img="{{ url('images/'.$row->photo_name) }}" data-name="{{ $row->name }}"
             data-address="{{ $row->rooms_address->city }}" data-host_img="{{ $row->users->profile_picture->src }}"></i>
        </label>
      </span>
                                                    </div>
                                                </div>
                                                <div class="panel-body panel-card-section">
                                                    <div class="media">
                                                        <a href="{{ url('users/show/'.$row->user_id) }}"
                                                           class="pull-right media-photo media-round card-profile-picture card-profile-picture-offset"
                                                           title="{{ $row->users->first_name }}">
                                                            <img src="{{ $row->users->profile_picture->src }}"
                                                                 height="60" width="60"
                                                                 alt="{{ $row->users->first_name }}">
                                                        </a>

                                                        <a href="{{ url('rooms/'.$row->id) }}" class="text-normal"
                                                           target="_blank">
                                                            <div title="{{ $row->name }}"
                                                        ...
                                                        <a href="{{ url('s/'.$row->rooms_address->city) }}"
                                                           class="text-muted text-normal">
                                                            <div class="listing-location text-truncate">{{ $row->rooms_address->city }}
                                                                , {{ $row->rooms_address->country_name }}</div>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
