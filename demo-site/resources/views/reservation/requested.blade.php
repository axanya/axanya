...
@section('main')
  ...
  <span class="show-sm">{{ trans('messages.payments.request_sent') }}!</span>
                            ...
                            @if($reservation_details->status == 'Accepted')
                                <h1 class="hide-sm space-1">
  <span>
  <span class="hide-sm">{{ trans('messages.payments.get_ready_for') }} {{ $reservation_details->rooms->rooms_address->city }}
      !</span>
  </span>
                                </h1>
                            @endif
  ...
  <span class="hide-sm">{{ trans('messages.payments.confirmed_reservation', ['first_name'=>$reservation_details->rooms->users->first_name,'email'=>$reservation_details->users->email]) }}</span>
                            ...
                            <form autocomplete="off" method="post" action="{{ url('reservation/itinerary_friends') }}"
                                  id="share-itinerary-form">
                                {!! Form::token() !!}
                                                                ...
                                                                <div class="row row-space-8 add-text-email">
                                                                    <div class="col-md-10 col-sm-12">
                                                                        <div class="friend-email space-2">
                                                                            <div data-email-tagging="false"
                                                                                 data-typeahead-type="recent"
                                                                                 class="email-input-typeahead-container">
  <span class="twitter-typeahead" style="position: relative; display: inline-block;">
  <input type="email" placeholder="{{ trans('messages.dashboard.email_address') }}" name="friend_address[]"
         autocomplete="none" class="typeahead  tt-input" spellcheck="false" dir="auto"
         style="position: relative; vertical-align: top;">
  <pre aria-hidden="true"
       style="position: absolute; visibility: hidden; white-space: pre; font-family: Circular,&quot;Helvetica Neue&quot;,Helvetica,Arial,sans-serif; font-size: 14px; font-style: normal; font-variant: normal; font-weight: 400; word-spacing: 0px; letter-spacing: 0px; text-indent: 0px; text-rendering: optimizelegibility; text-transform: none;">
  </pre>
  ...
  <div class="tt-dataset tt-dataset-email-typeahead">
                                                            ...
                                                            <a data-prevent-default="true" id="add_another"
                                                               href="javascript:void(0);">{{ trans('messages.payments.add_another') }}</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="space-md-4 space-sm-2">
                                                <button type="submit"
                                                        class="btn btn-primary">{{ trans('messages.lys.continue') }}</button>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-sm-5 col-sm-offset-1 listing-card hide-md hide-sm">
                                        <div>
                                            <div class="text-center">
                                                <div class="space-4">
                                                    <a target="_blank"
                                                       href="{{ url() }}/rooms/{{ $reservation_details->room_id }}">
                                                        <img src="{{ url('images/'.$reservation_details->rooms->photo_name) }}"
                                                             width="350">
                                                    </a>
                                                </div>
                                                ...
                                                <div class="col-sm-5 text-center">
                                                    <a class="link-reset"
                                                       href="{{ url() }}/users/show/{{ $reservation_details->host_id }}">
                                                        <div class="space-2">
                                                            <span style="background-image:url({{ $reservation_details->rooms->users->profile_picture->header_src }}); width:68px; height:68px; margin: 0 auto;"
                                                                  class="media-photo media-round"></span>
                                                        </div>
                                                        <div class="space-1">
                                                ...
                                                <div class="col-sm-7">
                                                    <h3 class="listing-card-spacing">
                                                        <a class="link-reset" target="_blank"
                                                           href="{{ url() }}/rooms/{{ $reservation_details->room_id }}">{{ $reservation_details->rooms->name }}</a>
                                                    </h3>
                                                    <h4 class="listing-card-spacing">{{ $reservation_details->dates }}</h4>
                                                    <h4 class="text-kazan">
                                                        @if($reservation_details->status == 'Accepted')
                                                            <div>
                                                                <span>{{ $reservation_details->rooms->rooms_address->address_line_1 }}</span>
                                                            </div>
                                                            <div><span>{{ $reservation_details->rooms->rooms_address->city }}
                                                                    , {{ $reservation_details->rooms->rooms_address->state }} {{ $reservation_details->rooms->rooms_address->postal_code }}</span>
                                                            </div>
                                                        @endif
                                                    </h4>
                                                </div>
