                            ...
                            @if($result->host_payout_email_id)
                                <form action="{{ url('admin/reservation/payout') }}" method="post">
                                    ...
                                    <div class="text-center">
                                        <button type="submit" class="btn btn-primary">Payout to
                                            Host({{ $result->currency->symbol }}{{ $result->admin_host_payout }})
                                        </button>
                                    </div>
                                </form>
                            @else
                                <div class="text-bold text-danger text-center">Yet, host doesn't enter his/her Payout
                                    preferences. <a
                                            href="{{ url('admin/reservation/need_payout_info/'.$result->id.'/host') }}">Send
                                        Email to Host</a></div>
                            @endif
                        @endif
                        ...
                        @if(($result->status == 'Declined' || $result->status == 'Cancelled' || $result->status == 'Expired') && $result->check_guest_payout != 'yes' && $result->admin_guest_payout != 0)
                            @if($result->guest_payout_email_id)
                                ...
                                <form action="{{ url('admin/reservation/payout') }}" method="post">
                                    ...
                                    <div class="text-center">
                                        <button type="submit" class="btn btn-primary">Payout to
                                            Guest({{ $result->currency->symbol }}{{ $result->admin_guest_payout }})
                                        </button>
                                    </div>
                                </form>
                            @else
                                <div class="text-bold text-danger text-center">Yet, guest doesn't enter his/her Payout
                                    preferences. <a
                                            href="{{ url('admin/reservation/need_payout_info/'.$result->id.'/guest') }}">Send
                                        Email to Guest</a></div>
                            @endif
                        @endif

                        @if($result->check_host_payout == 'yes')
                            <div class="text-bold text-success text-center">Host payout amount
                                ({{ $result->currency->symbol.$result->host_payout }}) transferred.
                            </div>
                        @endif
                        @if($result->check_guest_payout == 'yes')
                            <div class="text-bold text-success text-center">Guest payout amount
                                ({{ $result->currency->symbol.$result->guest_payout }}) transferred.
                            </div>
                        @endif
