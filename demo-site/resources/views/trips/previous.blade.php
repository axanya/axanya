    ...
    <main id="site-content" role="main">
        ...
        <span class="label label-orange label-{{ $previous_trip->status_color }}">
          {{ $previous_trip->status }}
                                            ...
                                            <td class="host"><a
                                                        href="{{ url() }}/users/show/{{ $previous_trip->host_id }}">{{ $previous_trip->rooms->users->full_name }}</a>
                                            </td>
                                                ...
                                                <ul class="unstyled button-list list-unstyled">
                                                    <li class="row-space-1">
                                                        <a class="button-steel"
                                                           href="{{ url() }}/z/q/{{ $previous_trip->id }}">{{ trans('messages.your_reservations.message_history') }}</a>
                                                    </li>
                                                    @if($previous_trip->status != "Cancelled" && $previous_trip->status != "Declined" && $previous_trip->status != "Expired")
                                                        <li class="row-space-1">
                                                            <a class="button-steel"
                                                               href="{{ url() }}/reservation/itinerary?code={{ $previous_trip->code }}">{{ trans('messages.your_trips.view_itinerary') }}</a>
                                                        </li>
                                                        <li class="row-space-1">
                                                            <a class="button-steel"
                                                               href="{{ url() }}/reservation/receipt?code={{ $previous_trip->code }}">{{ trans('messages.your_trips.view_receipt') }}</a>
                                                        </li>
                                                    @endif
                                                </ul>
                                            </td>
