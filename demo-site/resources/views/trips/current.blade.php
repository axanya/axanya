    ...
    <main id="site-content" role="main">
                                ...
                                <div class="row">
                                    <div class="col-8">
                                        <form method="get" class="row" action="{{ url() }}/s" accept-charset="UTF-8">
                                            <div style="margin:0;padding:0;display:inline"><input type="hidden"
                                                                                                  value="✓" name="utf8">
                                            </div>
                                            <div class="col-8">
                                                <input type="text"
        ...
        <span class="label label-orange label-{{ $pending_trip->status_color }}">
          {{ $pending_trip->status }}
                                            ...
                                            <td class="host"><a
                                                        href="{{ url() }}/users/show/{{ $pending_trip->host_id }}">{{ $pending_trip->rooms->users->full_name }}</a>
                                            </td>
                                            ...
                                            <td>
                                                <ul class="unstyled button-list list-unstyled">
                                                    <li class="row-space-1">
                                                        <a class="button-steel"
                                                           href="{{ url() }}/z/q/{{ $pending_trip->id }}">{{ trans('messages.your_reservations.message_history') }}</a>
                                                    </li>
                                                    <li class="row-space-1">
                                                    <!-- <a target="_blank" rel="nofollow" data-method="post" data-confirm="Are you sure that you want to cancel the request? Any money transacted will be refunded." class="button-steel" href="{{ url() }}/reservation/cancel?code={{ $pending_trip->code }}">{{ trans('messages.your_trips.cancel_request') }}</a> -->
                                                        <a rel="nofollow" data-method="post"
                                                           data-confirm="Are you sure that you want to cancel the request? Any money transacted will be refunded."
                                                           id="{{ $pending_trip->id }}-trigger" class="button-steel"
                                                           href="#">{{ trans('messages.your_trips.cancel_request') }}</a>
                                                    </li>
                                                </ul>
                                            </td>
                                        </tr>
        ...
        <span class="label label-orange label-{{ $current_trip->status_color }}">
          {{ $current_trip->status }}
                                            ...
                                            <td class="host"><a
                                                        href="{{ url() }}/users/show/{{ $current_trip->host_id }}">{{ $current_trip->rooms->users->full_name }}</a>
                                            </td>
                                            ...
                                            <td>
                                                <ul class="unstyled button-list list-unstyled">
                                                    <li class="row-space-1">
                                                        <a class="button-steel"
                                                           href="{{ url() }}/z/q/{{ $current_trip->id }}">{{ trans('messages.your_reservations.message_history') }}</a>
                                                    </li>
                                                    @if($current_trip->status != "Cancelled" && $current_trip->status != "Declined" && $current_trip->status != "Expired")
                                                        <li class="row-space-1">
                                                            <a class="button-steel"
                                                               href="{{ url() }}/reservation/itinerary?code={{ $current_trip->code }}">{{ trans('messages.your_trips.view_itinerary') }}</a>
                                                        </li>
                                                        <li class="row-space-1">
                                                            <a class="button-steel"
                                                               href="{{ url() }}/reservation/receipt?code={{ $current_trip->code }}">{{ trans('messages.your_trips.view_receipt') }}</a>
                                                        </li>
                                                        <li class="row-space-1">
                                                            <a class="button-steel" href="#"
                                                               id="{{$current_trip->id}}-trigger">{{ trans('messages.your_reservations.cancel') }}</a>
                                                        </li>
                                                    @endif
                                                </ul>
                                            </td>
        ...
        <span class="label label-orange label-{{ $upcoming_trip->status_color }}">
          {{ $upcoming_trip->status }}
                                            ...
                                            <td class="host"><a
                                                        href="{{ url() }}/users/show/{{ $upcoming_trip->host_id }}">{{ $upcoming_trip->rooms->users->full_name }}</a>
                                            </td>
                                            ...
                                            <td>
                                                <ul class="unstyled button-list list-unstyled">
                                                    <li class="row-space-1">
                                                        <a class="button-steel"
                                                           href="{{ url() }}/z/q/{{ $upcoming_trip->id }}">{{ trans('messages.your_reservations.message_history') }}</a>
                                                    </li>
                                                    @if($upcoming_trip->status != "Cancelled")
                                                        <li class="row-space-1">
                                                            <a class="button-steel"
                                                               href="{{ url() }}/reservation/itinerary?code={{ $upcoming_trip->code }}">{{ trans('messages.your_trips.view_itinerary') }}</a>
                                                        </li>
                                                        <li class="row-space-1">
                                                            <a class="button-steel"
                                                               href="{{ url() }}/reservation/receipt?code={{ $upcoming_trip->code }}">{{ trans('messages.your_trips.view_receipt') }}</a>
                                                        </li>
                                                        <li class="row-space-1">
                                                            <a class="button-steel" href="#"
                                                               id="{{$upcoming_trip->id}}-trigger">{{ trans('messages.your_reservations.cancel') }}</a>
                                                        </li>
                                                    @endif
                                                </ul>
                                            </td>
                ...
                <div class="modal-table">
                    <div class="modal-cell">
                        <div class="modal-content">
                            <form accept-charset="UTF-8" action="{{ url('trips/guest_cancel_reservation') }}"
                                  id="cancel_reservation_form" method="post" name="cancel_reservation_form">
                                {!! Form::token() !!}
                                <div class="panel-header">
                                    ...
                                    <div id="decline_reason_container">
                                        ...
                                        <div class="select">
                                            <select id="cancel_reason" name="cancel_reason">
                                                ...
                                                <option value="no_longer_need_accommodations">I no longer need
                                                    accommodations
                                                </option>
                                                <option value="travel_dates_changed">My travel dates changed</option>
                                                <option value="made_the_reservation_by_accident">I made the reservation
                                                    by accident
                                                </option>
                                                <option value="I_have_an_extenuating_circumstance">I have an extenuating
                                                    circumstance
                                                </option>
                                                <option value="my_host_needs_to_cancel">My host needs to cancel</option>
                                                <option value="uncomfortable_with_the_host">I’m uncomfortable with the
                                                    host
                                                </option>
                                                <option value="place_not_okay">The place isn't what I was expecting
                                                </option>
                                ...
                                <div class="panel-footer">
                                    ...
                                    <input class="btn btn-primary" id="cancel_submit" name="commit" type="submit"
