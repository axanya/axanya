...
class PaymentHelper
{
   ...
   */
    ...
    $result['service_fee']  = number_format(($percentage / 100) * $result['total_night_price']);
        ...
        {
            $result['service_fee'] = number_format(($percentage / 100) * SpecialOffer::find($special_offer_id)->price);
        }
        $result['service_fee'] = str_replace(',', '', $result['service_fee']);
    $result['host_fee']     = number_format(($host_fee_percentage / 100) * $result['total_night_price']);
        $result['host_fee']    = str_replace(',', '', $result['host_fee']);
    ...
    public function currency_convert($from = '', $to = '', $price)
    {
      if($from == '')
      {
        if(Session::get('currency'))
           ...
           $from = Currency::where('default_currency', 1)->value('code');
      }
      ...
      if($to == '')
      {
        if(Session::get('currency'))
           ...
           $to = Currency::where('default_currency', 1)->value('code');
      }

      $rate = Currency::whereCode($from)->value('rate');

      $usd_amount = $price / $rate;
      
      $session_rate = Currency::whereCode($to)->value('rate');
    ...
    {
    	$from_rate = Currency::whereCode($from)->value('rate');
    	$to_rate = Currency::whereCode($to)->value('rate');
