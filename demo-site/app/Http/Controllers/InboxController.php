        ...
        else if($reservation_details->rooms->user_id == Auth::user()->user()->id)
        {
            ...
            else if($request->template == 2)
            {
                ...
                else
                {
                    ...
                    $minimum_amount  = $this->payment_helper->currency_convert('USD', Session::get('currency'), 10);
                    $currency_symbol = Currency::whereCode(Session::get('currency'))->value('original_symbol');
                    ...
                    else
                    {
                        ...
                        //return json_encode(['success'=>'true','reservation_id'=>$reservation_details->id,'room_id'=>$request->pricing_room_id,'user_id'=>$reservation_details->user_id,'checkin'=>$request->pricing_checkin,'checkout'=>$request->pricing_checkout,'number_of_guests'=>$request->pricing_guests,'price'=>$request->pricing_price,'currency_code'=>Currency::find(1)->session_code,'type'=>'special_offer','created_at'=>'date("Y-m-d H:i:s")']);
                    }
                }
            }
