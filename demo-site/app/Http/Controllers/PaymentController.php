...
class PaymentController extends Controller 
{
    ...
    public function index(Request $request)
    {
        ...
        if(!$_POST && !$_GET)
        {
            ...
            $reservation_id = Session::get('payment_reservation_id');
        }
        elseif($_POST)
        {
            ...
            Session::put('payment_reservation_id', $request->reservation_id);
        ...
        if ($reservation_id != '')
        {
            ...
            if ( ! $reservation)
            {
                $this->helper->flash_message('error',
                    trans('messages.rooms.dates_not_available')); // Call flash message function
                return redirect('trips/current');
            }

            Session::put('payment_room_id', $reservation->room_id);
            Session::put('payment_checkin', date('d-m-Y', strtotime($reservation->checkin)));
            Session::put('payment_checkout', date('d-m-Y', strtotime($reservation->checkout)));
            Session::put('payment_number_of_guests', $reservation->number_of_guests);
            Session::put('payment_booking_type', 'instant_book');
            Session::put('payment_special_offer_id', $reservation->special_offer_id);
            Session::put('payment_reservation_id', $reservation->id);
        ...
        $data['special_offer_id'] = $request->special_offer_id;
        ...
        $pending_reservation_check = Reservation::where(['room_id'  => $data['room_id'],
        ...
        $data['paypal_price'] = $this->payment_helper->currency_convert($data['result']->rooms_price->code,
            PAYPAL_CURRENCY_CODE, $data['price_list']->total);

        $data['paypal_price_rate'] = $this->payment_helper->currency_rate($data['result']->rooms_price->currency_code,
    ...
    public function pre_accept(Request $request, EmailController $email_controller)
    {
        ...
        $data['price_list'] = json_decode($this->payment_helper->price_calculation($request->room_id, $request->checkin,
            $request->checkout, $request->number_of_guests, $request->special_offer_id));
        ...
        $rooms = Rooms::find(Session::get('payment_room_id'));

        $reservation = new Reservation;

        $reservation->room_id          = Session::get('payment_room_id');
        $reservation->host_id          = Rooms::find(Session::get('payment_room_id'))->user_id;
        $reservation->user_id          = Auth::user()->user()->id;
        $reservation->checkin          = $this->payment_helper->date_convert(Session::get('payment_checkin'));
        $reservation->checkout         = $this->payment_helper->date_convert(Session::get('payment_checkout'));
        $reservation->number_of_guests = Session::get('payment_number_of_guests');
        ...
        $message->room_id        = Session::get('payment_room_id');
        ...
        $this->helper->flash_message('success', trans('messages.rooms.pre-accept_request',
            ['first_name' => $rooms->users->first_name])); // Call flash message function
	...
	public function create_booking(Request $request)
	{
        ...
        $price_list = json_decode($this->payment_helper->price_calculation($request->room_id, $request->checkin,
            $request->checkout, $request->number_of_guests, $request->special_offer_id));
    ...
    {
        if (Session::get('payment_reservation_id')) $reservation = Reservation::find(Session::get('payment_reservation_id'));
        else
        $reservation = new Reservation;
        ...
        Session::forget('payment_reservation_id');
        Session::forget('coupon_code');
        Session::forget('coupon_amount');
        Session::forget('remove_coupon');
        Session::forget('manual_coupon');
    ...
    public function success(Request $request)
    {
        ...
        if (@$result['ACK'] == 'Success')
        {
            ...
            $this->helper->flash_message('success',
                trans('messages.payments.payment_success')); // Call flash message function
            return redirect('reservation/requested?code=' . $code);
        }
        else
        {
