...
class ReservationController extends Controller
{
     ...
     */
        ...
        $this->helper->flash_message('success',
            'Reservation Request has Successfully Pre-Accepted'); // Call flash message function
    ...
    public function decline(Request $request)
    {
        ...
        {
            $days = $this->get_days($reservation_details->checkin, $reservation_details->checkout);
            $rooms_price = RoomsPrice::where('room_id', $reservation_details->room_id)->value('night');
            // Update Calendar
            for($j=0; $j<count($days)-1; $j++)
            {
                $calendar_data = [
    ...
    public function requested(Request $request)
    {
        ...
        $data['reservation_details'] = Reservation::where('id', $request->code)->first();
        if ($data['reservation_details']=='') $data['reservation_details'] = Reservation::where('code', $request->code)->first();
    ...
    $reservation_details = Reservation::find($request->id);
        ...
        $days                             = $this->get_days($reservation_details->checkin,
